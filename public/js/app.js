...
async function refreshPortfolioValues(silent = false) {
  ...
  for (const item of _portfolio) {
    try {
      ...
      // ── 1. Check price_history_cache for today's price first ───────────────
      // This avoids calling the upstream API if the cron job (or a previous
      // manual refresh today) already fetched the price.
      try {
        const { data: cached } = await _sb
          ...
          .limit(1);
        if (cached?.length) priceUsd = parseFloat(cached[0].price);
      } catch (_) { /* cache miss — fall through to API */ }

      // ── 2. If not in cache, call the upstream API ──────────────────────────
      if (priceUsd == null) {
        ...
        priceUsd = extractPrice(match);
        if (priceUsd == null) continue;

        // Write to cache so future refreshes today skip the API call
        await _sb.from('price_history_cache').upsert({
          item_id:       String(item.item_id),
          type:          item.type,
          price:         Number(priceUsd),
          language:      item.language || 'english',
          recorded_date: today,
        }, { onConflict: 'item_id,type,language,recorded_date' });
      }
